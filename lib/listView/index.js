...
const ListView = React.createClass({
  ...
  getInitialState() {
    ...
    const cache = History.getCurrent().listView;

    this.listData = cache ? cache.listData : [];
    this.listGenData = cache ? cache.listGenData : [];
  ...
  componentWillMount() {
    // 没有缓存数据才从服务端获取
    if (!History.getCurrent().listView) {
      this.initData();
    } else {
      console.log('ListView从缓存里获取数据');
    }
  ...
  componentDidMount() {
    // 没有缓存数据才从服务端获取
    if (History.getCurrent().listView && this.listView) {
      this.listView.refs.listview.scrollTo(2, History.getCurrent().listView.offset);
    }
  },
  ...
  componentWillUnmount() {
    if (this.listView) {
      ...
      current.listView = Object.assign({}, this.state, {
        listData: this.listData,
        listGenData: this.listGenData,
        end: this.end,
  ...
  render() {
    ...
    const separator = (sectionID, rowID) => {
      let render = <div key={`${sectionID}-${rowID}`} style={{height: '0.2rem'}} />;
